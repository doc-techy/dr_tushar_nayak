        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-3 lg:gap-4 lg:max-w-5xl lg:mx-auto">
          {featuredServices.map((service) => (
            ...
            >
              ...
              {/* Image - 40% height on mobile, smaller on desktop */}
              ...
              <div className="relative space-y-1 sm:space-y-1.5 md:space-y-1.5 lg:space-y-1.5 p-2 sm:p-3 md:p-3 lg:p-3">
                ...
                <p className="text-gray-600 leading-relaxed line-clamp-2 text-[8px] sm:text-[10px] md:text-[10px] lg:text-xs">
                  {service.description}
                </p>

                {/* Highlights - visible on md+ */}
                <ul className="space-y-0.5 sm:space-y-1 md:space-y-1 lg:space-y-0.5 hidden md:block">
                  {service.highlights.slice(0, 2).map((highlight) => (
                    <li key={highlight} className="flex items-start gap-1 sm:gap-1.5 md:gap-1.5 lg:gap-1 text-[8px] sm:text-[10px] md:text-[10px] lg:text-[10px] text-gray-700">
              ...
              <div className="relative px-2 sm:px-3 md:px-3 lg:px-3 pb-2 sm:pb-3 md:pb-3 lg:pb-3 pt-0">
                <div className="flex items-center justify-between gap-1 pt-1.5 sm:pt-2 lg:pt-1.5 border-t border-gray-200 sm:border-t-2 lg:border-t group-hover:border-teal-200 transition-colors">
                  <span className="text-[8px] sm:text-[10px] md:text-[10px] lg:text-[10px] font-bold sm:font-black lg:font-bold uppercase tracking-wide text-brand-teal">
                    Learn More
                  </span>
                  <div className="flex w-4 h-4 sm:w-5 sm:h-5 md:w-5 md:h-5 lg:w-5 lg:h-5 rounded-full items-center justify-center bg-teal-100 transition-all duration-300 group-hover:bg-brand-teal group-hover:scale-110 flex-shrink-0">
                    <LuArrowRight className="h-2 w-2 sm:h-2.5 sm:w-2.5 md:h-2.5 md:w-2.5 lg:h-2.5 lg:w-2.5 text-brand-teal group-hover:text-white transition-colors group-hover:translate-x-0.5" />
                  </div>
                </div>
              </div>
            </Link>
